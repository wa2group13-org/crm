...
jq -n --arg d "$(<DOCKER.md)" '{"full_description":$d}' | curl -X PATCH -d @- -H "Content-Type: application/json" -H "Authorization: JWT $JWT" https://hub.docker.com/v2/repositories/wa2group13/crm
